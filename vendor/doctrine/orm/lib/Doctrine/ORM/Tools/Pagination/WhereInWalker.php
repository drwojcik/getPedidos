...
class WhereInWalker extends TreeWalkerAdapter {
	...
	public function walkSelectStatement(SelectStatement $AST) {
		...
		foreach ( $this->_getQueryComponents () as $dqlAlias => $qComp ) {
			$isParent = array_key_exists ( 'parent', $qComp ) && $qComp ['parent'] === null && $qComp ['nestingLevel'] == 0;
			if ($isParent) {
				$rootComponents [] = array (
						$dqlAlias => $qComp 
				);
		...
		if ($count > 0) {
			...
			$arithmeticExpression->simpleArithmeticExpression = new SimpleArithmeticExpression ( array (
					$pathExpression 
			) );
		...
		if ($AST->whereClause) {
			...
			$AST->whereClause = new WhereClause ( new ConditionalExpression ( array (
					new ConditionalTerm ( array (
							$conditionalPrimary 
					) ) 
			) ) );
